...
class StatsManager {
    ...
    async addVoiceStat(id, channel = "notfound", value) {
        return Stat.updateOne({Id: id}, {$inc: {[`Voice.${await tm.getDay()}.${channel}`]: value}}, {upsert: true}).exec();
    }
    ...
    async addMessageStat(id, channel = "notfound", value) {
        return Stat.updateOne({Id: id}, {$inc: {[`Message.${await tm.getDay()}.${channel}`]: value}}, {upsert: true}).exec();
    }
